...
// ── 最小化 SEO Fallback ──
// 仅在 Sanity 完全无数据时使用，确保页面有基本 SEO
...
function generateTocFromContent(
  blocks?: EggSpermFreezingPage["contentBlocks"]
) {
...
export default async function EggSpermFreezingPage() {
  ...
  const hero = {
    ...
    description:
      p?.heroDescription ||
      "天悦宝贝为有需求的人群提供冻卵、冻精生育力保存相关的专业咨询服务。",
  ...
  const tocItems =
    p?.tocItems?.length ? p.tocItems : generateTocFromContent(p?.contentBlocks);
  ...
  // 最终 CTA
  const finalCta = {
    title: p?.finalCtaTitle || "开始了解冻卵冻精方案",
    description:
      p?.finalCtaDescription ||
      "专业的咨询顾问随时为您服务，帮助您更清晰地了解技术、流程与方案选择。",
