...
export function SiteHeader({ siteSettings }: { siteSettings?: SiteSettings | null }) {
  ...
  return <header className={`z-50 w-full transition-all duration-300 ${transparent
    ...
    : "sticky top-0 border-b border-blue-100/80 bg-white/92 shadow-[0_8px_30px_rgba(40,95,160,.10)] backdrop-blur-xl"
  }`}>
      ...
      <nav className="hidden items-center gap-1 xl:flex" aria-label="主导航">
        {links.map(([label, href, hot]) => {
          ...
          return <Link key={href} href={href} className={`relative inline-flex items-center whitespace-nowrap rounded-full px-3 py-2 text-[13px] font-semibold transition-all ${active
            ...
            : transparent
              ? "text-[#1a2e52]/90 hover:bg-white/50 hover:text-[#1843a0]"
    ...
    {/* ── 移动端菜单 ── */}
    {open && <nav className="border-t border-blue-100 bg-white/96 px-5 py-4 shadow-xl backdrop-blur-xl xl:hidden">
      {links.map(([label, href, hot]) => {
        const active = pathname === href;
