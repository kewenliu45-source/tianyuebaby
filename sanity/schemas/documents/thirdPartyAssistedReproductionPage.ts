...
export const thirdPartyAssistedReproductionPage = defineType({
  ...
  fields: [
    ...
    defineField({ name: "heroImage", title: "Hero 图片", type: "imageWithAlt", description: "前台位置：页面顶部大图背景。是否建议改：必改。修改效果：直接影响页面视觉效果和用户第一印象。注意：建议尺寸 1920x800px，文件大小不超过 500KB，格式为 JPG 或 WebP；前台为响应式 object-cover 背景图，会随屏幕宽高裁剪，主体请放中间安全区。" }),
      ...
      of: [
        {
          ...
          fields: [
            ...
            defineField({ name: "image", title: "图片", type: "imageWithAlt", description: "前台位置：模块配图。是否建议改：建议改。修改效果：增强模块视觉效果。注意：图文混排建议 800x600px；如模块类型为全宽图片，前台按 16:7 显示，建议 1600x700 或 1920x840px；主体居中，文件大小不超过 300KB。" }),
            defineField({
              name: "imagePosition",
              title: "图片位置",
              type: "string",
              ...
              of: [
                {
                  ...
                  fields: [
                    ...
                    defineField({ name: "image", title: "图片", type: "imageWithAlt", description: "前台位置：子项配图。是否建议改：建议改。修改效果：为子项添加图片展示。注意：前台信息卡片多按 16:10 显示，建议 800x500 或 400x250px；如用于流程时间线宽图，建议 1600x700px。" }),
                  ],
                },
              ],
            }),
      ...
      of: [
        {
          ...
          fields: [
            ...
            defineField({ name: "image", title: "图片", type: "imageWithAlt", description: "前台位置：案例卡片的配图。是否建议改：建议改。修改效果：增强案例的视觉展示。注意：前台按 16:9 显示，建议 1200x675px，主体居中，注意保护隐私。" }),
          ],
        },
      ],
    }),
    ...
    defineField({ name: "finalCtaBackgroundImage", title: "背景图片", type: "imageWithAlt", description: "前台位置：CTA 区域的背景图片。是否建议改：建议改。修改效果：增强 CTA 区域的视觉吸引力。注意：建议尺寸 1920x600px，文件大小不超过 500KB，格式为 JPG 或 WebP；前台为响应式 object-cover 背景图，会随屏幕宽高裁剪，主体请放中间安全区。" }),
  ],
  preview: {
    prepare() {
