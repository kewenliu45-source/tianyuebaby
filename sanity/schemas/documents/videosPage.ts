...
export const videosPage = defineType({
  ...
  fields: [
    ...
    defineField({ name: "heroImage", title: "Hero 背景图", type: "imageWithAlt", description: "前台位置：页面顶部大图区域背景图。是否建议修改：建议改。修改效果：影响页面首屏视觉效果。注意：建议尺寸 1920×600 像素，大小不超过 500KB；前台为响应式 object-cover 背景图，会随屏幕宽高裁剪，主体请放中间安全区。" }),
    ...
    defineField({ name: "finalCtaBackgroundImage", title: "背景图片", type: "imageWithAlt", description: "前台位置：页面底部行动号召区域背景图。是否建议修改：建议改。修改效果：影响行动号召区域的视觉效果。注意：建议尺寸 1920×600 像素，大小不超过 500KB；前台为响应式 object-cover 背景图，会随屏幕宽高裁剪，主体请放中间安全区。" }),
