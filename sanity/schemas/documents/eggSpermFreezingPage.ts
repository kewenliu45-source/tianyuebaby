...
export const eggSpermFreezingPage = defineType({
  ...
  fields: [
    ...
    defineField({ name: "heroImage", title: "Hero 图片", type: "imageWithAlt", description: "前台位置：Hero 区域主图/背景图。建议改。修改效果：页面视觉第一印象。建议尺寸：1920x800px；前台为响应式 object-cover 背景图，会随屏幕宽高裁剪，主体请放中间安全区。" }),
      ...
      of: [
        {
          ...
          fields: [
            ...
            defineField({ name: "image", title: "图片", type: "imageWithAlt", description: "模块配图。建议改。图文混排建议 800x600px；如模块类型为全宽图片，前台按 16:7 显示，建议 1600x700 或 1920x840px；主体居中。" }),
            defineField({
              name: "imagePosition",
              title: "图片位置",
              type: "string",
              ...
              of: [
                {
                  ...
                  fields: [
                    ...
                    defineField({ name: "image", title: "图片", type: "imageWithAlt", description: "子项配图。可不改。前台信息卡片多按 16:10 显示，建议 800x500 或 400x250px；如用于流程时间线宽图，建议 1600x700px。" }),
                  ],
                },
              ],
            }),
      ...
      of: [
        {
          ...
          fields: [
            ...
            defineField({ name: "image", title: "图片", type: "imageWithAlt", description: "案例配图。建议改。前台按 16:9 显示，建议 1200x675px，主体居中。注意：需获得客户授权。" }),
          ],
        },
      ],
    }),
    ...
    defineField({ name: "finalCtaBackgroundImage", title: "背景图片", type: "imageWithAlt", description: "前台位置：最终 CTA 区域背景图。建议改。修改效果：增强视觉吸引力。建议尺寸：1920x600px；前台为响应式 object-cover 背景图，会随屏幕宽高裁剪，主体请放中间安全区。" }),
  ],
  preview: {
    prepare() {
