...
export const journey = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      type: "array",
      of: [{ type: "bannerSlide" }],
      description: "前台位置：助孕流程页面顶部轮播图。是否建议修改：可不改。修改效果：更换页面顶部展示的图片和文案。注意：桌面图建议 1920×600 像素，手机图建议 750×600 像素；前台为响应式 object-cover 背景图，会随屏幕宽高裁剪，主体请放中间安全区；该页面已从后台隐藏，非必要不修改。",
    }),
