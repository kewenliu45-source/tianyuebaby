...
export const ivfServicesPage = defineType({
  ...
  fields: [
    ...
    defineField({ name: "heroImage", title: "Hero 图片", type: "imageWithAlt", description: '前台位置：Hero 区域右侧主图。是否建议修改：建议改。修改效果：影响页面第一印象。注意：前台当前显示为右侧 4:3 图片，不是全宽背景；建议 1200×900 或 1600×1200px，500KB 以内，主体居中。' }),
    ...
    defineField({
      ...
      of: [{
        ...
        fields: [
          ...
          defineField({ name: "image", title: "图片", type: "imageWithAlt", description: "前台位置：旧版地区卡片配图（兼容字段，当前后台隐藏）。是否建议修改：可不改。注意：如启用旧版地区卡片，前台按 16:9 显示，建议 1200×675px，主体居中。" }),
    ...
    defineField({
      ...
      of: [{
        ...
        fields: [
          ...
          defineField({ name: "image", title: "地区图片", type: "imageWithAlt", description: '前台位置：区域卡片配图。是否建议修改：建议改。修改效果：增强区域的视觉展示。注意：前台地区卡片按 16:9 显示，建议 1200×675px，主体居中。' }),
    ...
    defineField({ name: "conversionBackgroundImage", title: "背景图", type: "imageWithAlt", description: '前台位置：转化模块背景大图。是否建议修改：建议改。修改效果：影响转化区视觉效果。注意：建议 1920×600px；前台为响应式 object-cover 背景图，会随屏幕宽高裁剪，主体请放中间安全区。' }),
    ...
    defineField({ name: "appointmentBackgroundImage", title: "预约背景图", type: "imageWithAlt", description: '前台位置：预约模块背景大图。是否建议修改：建议改。修改效果：影响预约区视觉效果。注意：建议 1920×600px；前台为响应式 object-cover 背景图，会随屏幕宽高裁剪，主体请放中间安全区。' }),
  ],
  preview: {
    prepare() {
