...
export const startJourney = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      type: "array",
      of: [{ type: "bannerSlide" }],
      description: "前台位置：页面顶部轮播图区域。是否建议修改：建议改。修改效果：更换或增删页面顶部的轮播幻灯片。注意：桌面图建议 1920×600 像素，手机图建议 750×600 像素；前台为响应式 object-cover 背景图，会随屏幕宽高裁剪，主体请放中间安全区；每张幻灯片需设置桌面图和 Alt 文本。",
    }),
