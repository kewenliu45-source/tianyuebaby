...
export const siteSettings = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      type: "image",
      description:
        '仅在页面未设置专属分享图、封面图或 Hero 图时使用。若主要用于微信缩略图，建议不小于 800×800；若用于标准社交分享，建议 1200×630。主体放在画面中央，系统和平台可能自动裁切。',
    }),
    ...
    defineField({
      ...
      type: "bannerSlide",
      description:
        '前台位置：未配置专属 Banner 的页面顶部轮播区。是否建议修改：建议改。修改效果：作为全站默认的顶部大图。注意：桌面图建议 1920×600px，手机图建议 750×600px；前台为响应式 object-cover 背景图，会随屏幕宽高裁剪，主体请放中间安全区。',
    }),
